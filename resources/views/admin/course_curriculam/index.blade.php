...
<div class="proprofimaincov">
    <div class="quizmaindata-cover">
        ...
        <div class="addadmsupbtnright">
            <a href="javascript:void(0)" id="create-course-curriculam">
                <i class='bx bxs-plus-circle'></i> Add Course Curriculam
            </a>
        </div>
    </div>
    <div class="mainquizans-cover">
        <div class="supadmmain_cover">
            <div class="supadmmain_maintbl">
                ...
                <table id="example" class="display table nowrap course_curriculam_datatable">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Title</th>
    ...
    @include('admin.course_curriculam.model')
</div>
<script>
    $(document).ready(function() {
        var table = $('.course_curriculam_datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('course_curriculam.index') }}",
                "data": function(d) {
...
<script type="text/javascript">
    var store = <?php echo json_encode(route('course_curriculam.store')) ?>;
    ...
    $("#course_curriculamform").validate({
        ignore: [],
        rules: {
            "title": {
                required: true,
        ...
        submitHandler: function() {
            $.ajax({
                data: $('#course_curriculamform').serialize(),
                url: store,
                type: "POST",
                dataType: 'json',
                success: function(data) {
                    if (data.success) {
                        ...
                        $('.course_curriculam_datatable').DataTable().draw(true);
                        setTimeout(function() {
                            $('#course_curriculamform').trigger("reset");
    ...
    $('body').on('click', '.edit-course-curriculam', function() {
        var course_curriculam_id = $(this).data('id');
        $('#alert-danger-form').hide();
        $('#alert-success-form').hide();
        $.get("{{ url('admin/'.$urlSlug.'/') }}" + '/' + course_curriculam_id + '/edit', function(data) {
            var base_url = '<?php echo url('/'); ?>';
            $('#ajaxModel').modal('show');
            $('#course_curriculam_id').val(data.data.id);
    ...
    $('body').on('click', '.delete-course-curriculam', function() {
        ...
        $.ajax({
            ...
            url: "{{ url('admin/'.$urlSlug.'/') }}" + '/' + course_curriculam_id,
            success: function(data) {
