...
<div class="proprofimaincov">
    ...
    <div class="mainquizans-cover">
        <div class="supadmmain_cover">
            <div class="supadmmain_maintbl">
                ...
                <table id="example" class="display table nowrap childcategory_datatable">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Title</th>
    ...
    @include('admin.child_category.model')
</div>
<script>
    $(document).ready(function() {
        var table = $('.childcategory_datatable').DataTable({
            processing: true,
            serverSide: true,
...
<script type="text/javascript">
    var store = <?php echo json_encode(route('child_category.store')) ?>;
    ...
    $("#childcategoryform").validate({
        ...
        submitHandler: function() {
            $.ajax({
                data: $('#childcategoryform').serialize(),
                url: store,
                type: "POST",
                dataType: 'json',
                success: function(data) {
                    if (data.success) {
                        ...
                        $('.childcategory_datatable').DataTable().draw(true);
                        setTimeout(function() {
    ...
    $('body').on('click', '.edit-category', function() {
        ...
        $('#alert-danger-form').hide();
        $('#alert-success-form').hide();
        $.get("{{ url('admin/'.$urlSlug.'/') }}" + '/' + childcategory_id + '/edit', function(data) {
            var base_url = '<?php echo url('/'); ?>';
            $('#ajaxModel').modal('show');
            $('#childcategory_id').val(data.data.id);
    ...
    $('body').on('click', '.delete-category', function() {
        ...
        $.ajax({
            ...
            url: "{{ url('admin/'.$urlSlug.'/') }}" + '/' + childcategory_id,
            success: function(data) {
