...
<div class="proprofimaincov">
    ...
    <div class="mainquizans-cover">
        <div class="supadmmain_cover">
            <div class="supadmmain_maintbl">
                ...
                <table id="example" class="display table nowrap course_type_datatable">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Name</th>
    ...
    @include('admin.course_type.model')
</div>
<script>
    $(document).ready(function() {
        var table = $('.course_type_datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('course_type.index') }}",
            autoWidth: true,
            dom: 'Bfrtip',
            order: [ [0, 'desc'] ],
            buttons: [{
...
<script type="text/javascript">
    var store = <?php echo json_encode(route('course_type.store')) ?>;
    ...
    $("#course_typeform").validate({
        ...
        submitHandler: function() {
            $.ajax({
                data: $('#course_typeform').serialize(),
                url: store,
                type: "POST",
                dataType: 'json',
                success: function(data) {
                    if (data.success) {
                        ...
                        $('.course_type_datatable').DataTable().draw(true);
                        setTimeout(function() {
                            $('#course_typeform').trigger("reset");
    ...
    $('#create-course-type').click(function() {
        $('#course_type_id').val('');
        $('#course_typeform').trigger("reset");
    ...
    $('body').on('click', '.edit-course-type', function() {
        var course_type_id = $(this).data('id');
        $('#alert-danger-form').hide();
        $('#alert-success-form').hide();
        $.get("{{ url('admin/'.$urlSlug.'/') }}" + '/' + course_type_id + '/edit', function(data) {
            var base_url = '<?php echo url('/'); ?>';
            $('#ajaxModel').modal('show');
            $('#course_type_id').val(data.data.id);
    ...
    $('body').on('click', '.delete-course-type', function() {
        ...
        $.ajax({
            ...
            url: "{{ url('admin/'.$urlSlug.'/') }}" + '/' + course_type_id,
            success: function(data) {
