...
<div class="proprofimaincov">
    <div class="quizmaindata-cover">
        ...
        <div class="addadmsupbtnright">
            <a href="javascript:void(0)" id="create-subject">
                <i class='bx bxs-plus-circle'></i> Add Subject
            </a>
        </div>
    </div>
    <div class="mainquizans-cover">
        <div class="supadmmain_cover">
            <div class="supadmmain_maintbl">
                ...
                <table id="example" class="display table nowrap subject_datatable">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Title</th>
    ...
    @include('admin.subject.model')
</div>
<script>
    $(document).ready(function() {
        var table = $('.subject_datatable').DataTable({
            processing: true,
            serverSide: true,
...
<script type="text/javascript">
    var store = <?php echo json_encode(route('subject.store')) ?>;
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        ...
                        $('.subject_datatable').DataTable().draw(true);
                        setTimeout(function() {
    ...
    $('body').on('click', '.edit-subject', function() {
        ...
        $('#alert-danger-form').hide();
        $('#alert-success-form').hide();
        $.get("{{ url('admin/'.$urlSlug.'/') }}" + '/' + subject_id + '/edit', function(data) {
    ...
    $('body').on('click', '.delete-subject', function() {
        ...
        $.ajax({
            ...
            url: "{{ url('admin/'.$urlSlug.'/') }}" + '/' + subject_id,
            success: function(data) {
